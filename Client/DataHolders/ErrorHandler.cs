...
namespace CloneCAD.Client.DataHolders
{
    public class ErrorHandler
    {
        ...
        public void Error(string localeKey, params object[] objs) =>
            MessageBox.Show(Locale[localeKey, objs], @"CloneCAD", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            if (Locale == null)
                MessageBox.Show(@"The error has been saved to a log (" + Common.Functions.ExceptionHandlerBackend(e) + @").\nPlease post an issue on GitHub and put the contents of the log in a code block.", @"CloneCAD", MessageBoxButtons.OK, MessageBoxIcon.Error);
            else
                MessageBox.Show(Locale["UnexpectedError"].StartsWith("LOCALE ERROR (")
                    ? "The error has been saved to a log (" + Common.Functions.ExceptionHandlerBackend(e) + ").\nPlease post an issue on GitHub and put the contents of the log in a code block."
                    : Locale["UnexpectedError", Common.Functions.ExceptionHandlerBackend(e)], @"CloneCAD", MessageBoxButtons.OK, MessageBoxIcon.Error);
